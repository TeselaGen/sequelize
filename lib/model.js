  ...
  _createClass(Model, null, [{
    ...
    value: function _paranoidClause(model, options) {
      ...
      if (options.include) {
        ...
        try {
          for (var _iterator = options.include[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var deletedAtObject = {};
      var deletedAtDefaultValue = deletedAtAttribute.hasOwnProperty('defaultValue') ? deletedAtAttribute.defaultValue : null;
    ...
    value: function _addDefaultAttributes() {
      ...
      var existingAttributes = Utils._.clone(this.rawAttributes);
      this.rawAttributes = {};

      Utils._.each(head, function (value, attr) {
        _this.rawAttributes[attr] = value;
      });
      ...
      Utils._.each(tail, function (value, attr) {
        if (Utils._.isUndefined(_this.rawAttributes[attr])) {
          _this.rawAttributes[attr] = value;
        }
      });
    ...
    key: '_findAutoIncrementField',
    value: function _findAutoIncrementField() {
    ...
    key: '_conformOptions',
    value: function _conformOptions(options, self) {
      var _this2 = this;
      ...
      options.include = options.include.map(function (include) {
        return _this2._conformInclude(include, self);
      });
    }
  }, {
    ...
    value: function _validateIncludedElements(options, tableNames) {
      ...
      options.include = options.include.map(function (include) {
        include = _this3._conformInclude(include);
        include.parent = options;

        _this3._validateIncludedElement.call(options.model, include, tableNames, options);
      ...
      try {
        for (var _iterator3 = options.include[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          var include = _step3.value;
    ...
    value: function _getIncludedAssociation(targetModel, targetAlias) {
      ...
      if (associations.length === 0) {
        ...
        if (!association) {
          if (targetAlias) {
            throw new sequelizeErrors.EagerLoadingError(targetModel.name + ' is associated to ' + this.name + ' using an alias. ' + ('You\'ve included an alias (' + targetAlias + '), but it does not match the alias defined in your association.'));
          } else {
    ...
    value: function _expandIncludeAll(options) {
      ...
      Utils._.forEach(includes, function (include) {
        _this4._expandIncludeAll.call(include.model, include);
      });
    }
    ...
    value: function init(attributes, options) {
      ...
      _.each(options.validate, function (validator, validatorType) {
        if (_.includes(Utils._.keys(attributes), validatorType)) {
          throw new Error('A model validator function must not have the same name as a field. Model: ' + _this5.name + ', field/validation name: ' + validatorType);
        }

        if (!_.isFunction(validator)) {
          throw new Error('Members of the validate option must be functions. Model: ' + _this5.name + ', error with validate member ' + validatorType);
        }
      });

      this.attributes = this.rawAttributes = _.mapValues(attributes, function (attribute, name) {

        attribute = _this5.sequelize.normalizeAttribute(attribute);
      ...
      this._isReadOnlyAttribute = Utils._.memoize(function (key) {
        return _this5._hasReadOnlyAttributes && _this5._readOnlyAttributes.indexOf(key) !== -1;
      });
      ...
      _.each(this.options.scopes, function (scope) {
        if (_.isPlainObject(scope)) {
          _this5._conformOptions(scope, _this5);
        }
      });

      this.options.indexes = this.options.indexes.map(function (index) {
        index = _this5._conformIndex(index);
        ...
        if (index.unique && index.fields) {
          ...
          try {
            for (var _iterator4 = index.fields[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
              var field = _step4.value;

              var fieldName = typeof field === 'string' ? field : field.name || field.attribute;
              if (_this5.rawAttributes[fieldName]) {
                _this5.rawAttributes[fieldName].unique = _this5.attributes[fieldName].unique = index.name || index.unique;
              }
            }
          } catch (err) {
    ...
    value: function refreshAttributes() {
      ...
      Utils._.each(['get', 'set'], function (type) {
        ...
        var funcs = Utils._.clone(Utils._.isObject(_this6.options[opt]) ? _this6.options[opt] : {});
        var _custom = type === 'get' ? _this6.prototype._customGetters : _this6.prototype._customSetters;
        ...
        Utils._.each(_this6.rawAttributes, function (options, attribute) {
      ...
      _.each(this.rawAttributes, function (definition, name) {
        definition.type = _this6.sequelize.normalizeDataType(definition.type);

        definition.Model = _this6;
        ...
        if (definition.primaryKey === true) {
          _this6.primaryKeys[name] = definition;
        }

        _this6.fieldRawAttributesMap[definition.field] = definition;

        if (definition.type instanceof DataTypes.BOOLEAN) {
          _this6._booleanAttributes.push(name);
        } else if (definition.type instanceof DataTypes.DATE || definition.type instanceof DataTypes.DATEONLY) {
          _this6._dateAttributes.push(name);
        } else if (definition.type instanceof DataTypes.HSTORE || DataTypes.ARRAY.is(definition.type, DataTypes.HSTORE)) {
          _this6._hstoreAttributes.push(name);
        } else if (definition.type instanceof DataTypes.RANGE || DataTypes.ARRAY.is(definition.type, DataTypes.RANGE)) {
          _this6._rangeAttributes.push(name);
        } else if (definition.type instanceof DataTypes.JSON) {
          _this6._jsonAttributes.push(name);
        } else if (definition.type instanceof DataTypes.VIRTUAL) {
          _this6._virtualAttributes.push(name);
        } else if (definition.type instanceof DataTypes.GEOMETRY) {
          _this6._geometryAttributes.push(name);
        }

        if (definition.hasOwnProperty('defaultValue')) {
          _this6._defaultValues[name] = Utils._.partial(Utils.toDefaultValue, definition.defaultValue);
        }

        if (definition.hasOwnProperty('unique') && definition.unique !== false) {
          var idxName = void 0;
          if (_typeof(definition.unique) === 'object' && definition.unique.hasOwnProperty('name')) {
            idxName = definition.unique.name;
          } else if (typeof definition.unique === 'string') {
            idxName = definition.unique;
          } else {
            idxName = _this6.tableName + '_' + name + '_unique';
          }

          var idx = _this6.options.uniqueKeys[idxName] || { fields: [] };
          ...
          _this6.options.uniqueKeys[idxName] = idx;
        }

        if (definition.hasOwnProperty('validate')) {
          _this6.prototype.validators[name] = definition.validate;
        }

        if (definition.index === true && definition.type instanceof DataTypes.JSONB) {
          _this6.options.indexes.push({
      ...
      try {
        for (var _iterator5 = Object.keys(attributeManipulation)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
      ...
      this.prototype._isAttribute = Utils._.memoize(function (key) {
        return _this6.prototype.attributes.indexOf(key) !== -1;
      });
      ...
      this._isPrimaryKey = Utils._.memoize(function (key) {
        return _this6.primaryKeyAttributes.indexOf(key) !== -1;
      });
    }
    ...
    value: function sync(options) {
      ...
      var attributes = this.tableAttributes;

      return Promise.try(function () {
        if (options.hooks) {
          return _this7.runHooks('beforeSync', options);
        }
      }).then(function () {
        if (options.force) {
          return _this7.drop(options);
        }
      }).then(function () {
        return _this7.QueryInterface.createTable(_this7.getTableName(options), attributes, options, _this7);
      }).then(function () {
        if (options.alter) {
          return _this7.QueryInterface.describeTable(_this7.getTableName(options)).then(function (columns) {
            var changes = []; // array of promises to run
            _.each(attributes, function (columnDesc, columnName) {
              if (!columns[columnName]) {
                changes.push(function () {
                  return _this7.QueryInterface.addColumn(_this7.getTableName(options), columnName, attributes[columnName]);
                });
              }
            });
            _.each(columns, function (columnDesc, columnName) {
              if (!attributes[columnName]) {
                changes.push(function () {
                  return _this7.QueryInterface.removeColumn(_this7.getTableName(options), columnName, options);
                });
              } else if (!attributes[columnName].primaryKey) {
                changes.push(function () {
                  return _this7.QueryInterface.changeColumn(_this7.getTableName(options), columnName, attributes[columnName]);
                });
              }
            });
            return changes.reduce(function (p, fn) {
              return p.then(fn);
            }, Promise.resolve());
          });
        }
      }).then(function () {
        return _this7.QueryInterface.showIndex(_this7.getTableName(options), options);
      }).then(function (indexes) {
        // Assign an auto-generated name to indexes which are not named by the user
        _this7.options.indexes = _this7.QueryInterface.nameIndexes(_this7.options.indexes, _this7.tableName);

        indexes = _.filter(_this7.options.indexes, function (item1) {
          return !_.some(indexes, function (item2) {
            return item1.name === item2.name;
          });
        }).sort(function (index1, index2) {
          if (_this7.sequelize.options.dialect === 'postgres') {
        ...
        return Promise.map(indexes, function (index) {
          return _this7.QueryInterface.addIndex(_this7.getTableName(options), _.assign({
            logging: options.logging,
            benchmark: options.benchmark,
            transaction: options.transaction
          }, index), _this7.tableName);
        });
      }).then(function () {
        if (options.hooks) {
          return _this7.runHooks('afterSync', options);
        }
      }).return(this);
    }
    ...
    value: function schema(_schema, options) {
      ...
      var clone = function (_ref) {
        _inherits(clone, _ref);

        function clone() {
          _classCallCheck(this, clone);

          return _possibleConstructorReturn(this, (clone.__proto__ || Object.getPrototypeOf(clone)).apply(this, arguments));
        }
    ...
    value: function scope(option) {
      var self = function (_ref2) {
        _inherits(self, _ref2);
      ...
      try {
        for (var _iterator6 = options[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
          ...
          if (_.isPlainObject(_option)) {
            if (_option.method) {
              if (Array.isArray(_option.method) && !!self.options.scopes[_option.method[0]]) {
                scopeName = _option.method[0];
                scope = self.options.scopes[scopeName].apply(self, _option.method.slice(1));
              } else if (self.options.scopes[_option.method]) {
                scopeName = _option.method;
            ...
            if (_option === 'defaultScope' && _.isPlainObject(self.options.defaultScope)) {
          ...
          if (scope) {
            _.assignWith(self._scope, scope, function (objectValue, sourceValue, key) {
              if (key === 'where') {
    ...
    value: function findAll(options) {
      ...
      return Promise.try(function () {
        _this10._injectScope(options);

        if (options.hooks) {
          return _this10.runHooks('beforeFind', options);
        }
      }).then(function () {
        _this10._conformOptions(options, _this10);
        _this10._expandIncludeAll(options);

        if (options.hooks) {
          return _this10.runHooks('beforeFindAfterExpandIncludeAll', options);
        }
      }).then(function () {
        if (options.include) {
          options.hasJoin = true;

          _this10._validateIncludedElements(options, tableNames);

          // If we're not raw, we have to make sure we include the primary key for deduplication
          if (options.attributes && !options.raw && _this10.primaryKeyAttribute && options.attributes.indexOf(_this10.primaryKeyAttribute) === -1) {
            options.originalAttributes = options.attributes;
            options.attributes = [_this10.primaryKeyAttribute].concat(options.attributes);
          }
        }

        if (!options.attributes) {
          options.attributes = Object.keys(_this10.tableAttributes);
        }

        // whereCollection is used for non-primary key updates
        _this10.options.whereCollection = options.where || null;

        Utils.mapFinderOptions(options, _this10);

        options = _this10._paranoidClause(_this10, options);

        if (options.hooks) {
          return _this10.runHooks('beforeFindAfterOptions', options);
        }
      }).then(function () {
        originalOptions = Utils.cloneDeep(options);
        options.tableNames = Object.keys(tableNames);
        return _this10.QueryInterface.select(_this10, _this10.getTableName(options), options);
      }).tap(function (results) {
        if (options.hooks) {
          return _this10.runHooks('afterFind', results, options);
        }
    ...
    value: function warnOnInvalidOptions(options, validColumnNames) {
      ...
      var validQueryKeywords = ['where', 'attributes', 'paranoid', 'include', 'order', 'limit', 'offset', 'transaction', 'lock', 'raw', 'logging', 'benchmark', 'having', 'searchPath', 'rejectOnEmpty', 'plain', 'scope', 'group', 'through', 'defaults', 'distinct', 'primary', 'exception', 'type', 'hooks', 'force', 'name'];

      var unrecognizedOptions = _.difference(Object.keys(options), validQueryKeywords);
    ...
    value: function _findSeparate(results, options) {
      ...
      return Promise.map(options.include, function (include) {
        if (!include.separate) {
          return Model._findSeparate(results.reduce(function (memo, result) {
        ...
        return include.association.get(results, _.assign({}, _.omit(options, 'include', 'attributes', 'order', 'where', 'limit', 'plain'), _.omit(include, 'parent', 'association', 'as'))).then(function (map) {
          ...
          try {
            for (var _iterator7 = results[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
              ...
              result.set(include.association.as, map[result.get(include.association.source.primaryKeyAttribute)], {
                raw: true
              });
            }
          } catch (err) {
    ...
    key: 'findById',
    value: function findById(param, options) {
    ...
    value: function aggregate(attribute, aggregateFunction, options) {
      ...
      var field = attrOptions && attrOptions.field || attribute;
    ...
    value: function count(options) {
      ...
      return Promise.try(function () {
        ...
        if (options.hooks) {
          return _this11.runHooks('beforeCount', options);
        }
      }).then(function () {
        var col = options.col || '*';
        if (options.include) {
          col = _this11.name + '.' + (options.col || _this11.primaryKeyField);
        }

        Utils.mapOptionFieldNames(options, _this11);
        ...
        return _this11.aggregate(col, 'count', options);
      });
    }
    ...
    value: function findAndCount(options) {
      ...
      var countQuery = this.count(countOptions);
      var findQuery = this.findAll(options);
      ...
      return countQuery.then(function (count) {
        ...
        return findQuery.then(function (results) {
          return {
            count: count || 0,
            rows: results
          };
    ...
    value: function bulkBuild(valueSets, options) {
      ...
      return valueSets.map(function (values) {
        return _this12.build(values, options);
      });
    }
    ...
    value: function findOrBuild(options) {
      ...
      return this.find(options).then(function (instance) {
        if (instance === null) {
    ...
    value: function findOrCreate(options) {
      ...
      if (options.transaction === undefined && this.sequelize.constructor._cls) {
        var t = this.sequelize.constructor._cls.get('transaction');
        if (t) {
          options.transaction = t;
        }
      }

      var internalTransaction = !options.transaction;
      var values = void 0;
      var transaction = void 0;

      // Create a transaction or a savepoint, depending on whether a transaction was passed in
      return this.sequelize.transaction(options).then(function (t) {
        transaction = t;
        options.transaction = t;

        return _this14.findOne(_.defaults({ transaction: transaction }, options));
        ...
        return _this14.create(values, options).then(function (instance) {
          if (instance.get(_this14.primaryKeyAttribute, { raw: true }) === null) {
            // If the query returned an empty result for the primary key, we know that this was actually a unique constraint violation
            throw new _this14.sequelize.UniqueConstraintError();
          }

          return [instance, true];
        }).catch(_this14.sequelize.UniqueConstraintError, function (err) {
          var whereFields = Object.keys(options.where).map(function (name) {
            return _this14.rawAttributes[name].field || name;
          });
          var defaultFields = options.defaults && Object.keys(options.defaults).map(function (name) {
            return _this14.rawAttributes[name].field || name;
          });
          ...
          if (_.intersection(Object.keys(err.fields), whereFields).length) {
            _.each(err.fields, function (value, key) {
              var name = _this14.fieldRawAttributesMap[key].fieldName;
              if (value.toString() !== options.where[name].toString()) {
                throw new Error(_this14.name + '#findOrCreate: value used for ' + name + ' was not equal for both the find and the create calls, \'' + options.where[name] + '\' vs \'' + value + '\'');
              }
            });
          }

          // Someone must have created a matching instance inside the same transaction since we last did a find. Let's find it!
          return _this14.findOne(_.defaults({
    ...
    value: function findCreateFind(options) {
      ...
      return this.findOne(options).then(function (result) {
        ...
        return _this15.create(values, options).then(function (result) {
          return [result, true];
        }).catch(_this15.sequelize.UniqueConstraintError, function () {
          return _this15.findOne(options).then(function (result) {
            return [result, false];
          });
        });
      });
    ...
    value: function upsert(values, options) {
      ...
      var hadPrimary = this.primaryKeyField in values || this.primaryKeyAttribute in values;
      ...
      return instance.validate(options).then(function () {
        // Map field names
        var updatedDataValues = _.pick(instance.dataValues, Object.keys(instance._changed));
        var insertValues = Utils.mapValueFieldNames(instance.dataValues, instance.attributes, _this16);
        var updateValues = Utils.mapValueFieldNames(updatedDataValues, options.fields, _this16);
        var now = Utils.now(_this16.sequelize.options.dialect);

        // Attach createdAt
        if (createdAtAttr && !updateValues[createdAtAttr]) {
          insertValues[createdAtAttr] = _this16._getDefaultTimestamp(createdAtAttr) || now;
        }
        if (updatedAtAttr && !insertValues[updatedAtAttr]) {
          insertValues[updatedAtAttr] = updateValues[updatedAtAttr] = _this16._getDefaultTimestamp(updatedAtAttr) || now;
        }

        // Build adds a null value for the primary key, if none was given by the user.
        // We need to remove that because of some Postgres technicalities.
        if (!hadPrimary && _this16.primaryKeyAttribute && !_this16.rawAttributes[_this16.primaryKeyAttribute].defaultValue) {
          delete insertValues[_this16.primaryKeyField];
          delete updateValues[_this16.primaryKeyField];
        }

        return Promise.try(function () {
          if (options.hooks) {
            return _this16.runHooks('beforeUpsert', values, options);
          }
        }).then(function () {
          return _this16.QueryInterface.upsert(_this16.getTableName(options), insertValues, updateValues, instance.where(), _this16, options);
        }).tap(function (result) {
          if (options.hooks) {
            return _this16.runHooks('afterUpsert', result, options);
          }
        });
      });
    }
    ...
    value: function bulkCreate(records, options) {
      ...
      var instances = records.map(function (values) {
        return _this17.build(values, { isNewRecord: true });
      });

      return Promise.try(function () {
        // Run before hook
        if (options.hooks) {
          return _this17.runHooks('beforeBulkCreate', instances, options);
        }
      }).then(function () {
        // Validate
        if (options.validate) {
          var errors = new Promise.AggregateError();
          var validateOptions = _.clone(options);
        ...
        try {
          for (var _iterator8 = instances[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
            ...
            instance.dataValues = Utils.mapValueFieldNames(values, options.fields, _this17);
          }
        } catch (err) {
        ...
        if (options.individualHooks) {
          ...
          records = instances.map(function (instance) {
            return Utils._.omit(instance.dataValues, _this17._virtualAttributes);
          });

          // Map attributes for serial identification
          var attributes = {};
          for (var attr in _this17.tableAttributes) {
            attributes[_this17.rawAttributes[attr].field] = _this17.rawAttributes[attr];
          }

          return _this17.QueryInterface.bulkInsert(_this17.getTableName(options), records, options, attributes).then(function (results) {
            if (Array.isArray(results)) {
              results.forEach(function (result, i) {
                instances[i].set(_this17.primaryKeyAttribute, result[_this17.rawAttributes[_this17.primaryKeyAttribute].field], { raw: true });
              });
            }
            return results;
          });
        ...
        instances.forEach(function (instance) {
          for (var attr in _this17.rawAttributes) {
            if (_this17.rawAttributes[attr].field && instance.dataValues[_this17.rawAttributes[attr].field] !== undefined && _this17.rawAttributes[attr].field !== attr) {
              instance.set(attr, instance.dataValues[_this17.rawAttributes[attr].field]);
              delete instance.dataValues[_this17.rawAttributes[attr].field];
            }
          }
        ...
        // Run after hook
        if (options.hooks) {
          return _this17.runHooks('afterBulkCreate', instances, options);
        }
      }).then(function () {
        return instances;
      });
    ...
    value: function destroy(options) {
      ...
      return Promise.try(function () {
        // Run before hook
        if (options.hooks) {
          return _this18.runHooks('beforeBulkDestroy', options);
        }
      }).then(function () {
        // Get daos and run beforeDestroy hook on each record individually
        if (options.individualHooks) {
          return _this18.findAll({ where: options.where, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark }).map(function (instance) {
            return _this18.runHooks('beforeDestroy', instance, options).then(function () {
              return instance;
            });
          }).then(function (_instances) {
            instances = _instances;
        ...
        if (_this18._timestampAttributes.deletedAt && !options.force) {
          // Set query type appropriately when running soft delete
          options.type = QueryTypes.BULKUPDATE;

          var attrValueHash = {};
          var deletedAtAttribute = _this18.rawAttributes[_this18._timestampAttributes.deletedAt];
          var field = _this18.rawAttributes[_this18._timestampAttributes.deletedAt].field;
          var where = {};

          where[field] = deletedAtAttribute.hasOwnProperty('defaultValue') ? deletedAtAttribute.defaultValue : null;

          attrValueHash[field] = Utils.now(_this18.sequelize.options.dialect);
          return _this18.QueryInterface.bulkUpdate(_this18.getTableName(options), attrValueHash, _.defaults(where, options.where), options, _this18.rawAttributes);
        } else {
          return _this18.QueryInterface.bulkDelete(_this18.getTableName(options), options.where, options, _this18);
        }
      }).tap(function () {
        // Run afterDestroy hook on each record individually
        if (options.individualHooks) {
          return Promise.map(instances, function (instance) {
            return _this18.runHooks('afterDestroy', instance, options);
          });
        }
      }).tap(function () {
        // Run after hook
        if (options.hooks) {
          return _this18.runHooks('afterBulkDestroy', options);
        }
      });
    }
    ...
    value: function restore(options) {
      ...
      return Promise.try(function () {
        // Run before hook
        if (options.hooks) {
          return _this19.runHooks('beforeBulkRestore', options);
        }
      }).then(function () {
        // Get daos and run beforeRestore hook on each record individually
        if (options.individualHooks) {
          return _this19.findAll({ where: options.where, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark, paranoid: false }).map(function (instance) {
            return _this19.runHooks('beforeRestore', instance, options).then(function () {
              return instance;
            });
          }).then(function (_instances) {
            instances = _instances;
        ...
        // Run undelete query
        var attrValueHash = {};
        var deletedAtCol = _this19._timestampAttributes.deletedAt;
        var deletedAtAttribute = _this19.rawAttributes[deletedAtCol];
        var deletedAtDefaultValue = deletedAtAttribute.hasOwnProperty('defaultValue') ? deletedAtAttribute.defaultValue : null;

        attrValueHash[deletedAtAttribute.field || deletedAtCol] = deletedAtDefaultValue;
        options.omitNull = false;
        return _this19.QueryInterface.bulkUpdate(_this19.getTableName(options), attrValueHash, options.where, options, _this19._timestampAttributes.deletedAt);
      }).tap(function () {
        // Run afterDestroy hook on each record individually
        if (options.individualHooks) {
          return Promise.map(instances, function (instance) {
            return _this19.runHooks('afterRestore', instance, options);
          });
        }
      }).tap(function () {
        // Run after hook
        if (options.hooks) {
          return _this19.runHooks('afterBulkRestore', options);
        }
      });
    }
    ...
    value: function update(values, options) {
      ...
      var instances = void 0;
      var valuesUse = void 0;

      return Promise.try(function () {
        // Validate
        if (options.validate) {
          var build = _this20.build(values);
          build.set(_this20._timestampAttributes.updatedAt, values[_this20._timestampAttributes.updatedAt], { raw: true });
          ...
          options.skip = Utils._.difference(Object.keys(_this20.attributes), Object.keys(values));
        ...
        if (options.hooks) {
          ...
          return _this20.runHooks('beforeBulkUpdate', options).then(function () {
        ...
        if (options.individualHooks) {
          return _this20.findAll({ where: options.where, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark }).then(function (_instances) {
            instances = _instances;
            if (!instances.length) {
              return [];
            }
            ...
            return Promise.map(instances, function (instance) {
              ...
              return _this20.runHooks('beforeUpdate', instance, options).then(function () {
                if (!different) {
        ...
        valuesUse = Utils.mapValueFieldNames(valuesUse, options.fields, _this20);
        options = Utils.mapOptionFieldNames(options, _this20);
        options.hasTrigger = _this20.options ? _this20.options.hasTrigger : false;

        // Run query to update all rows
        return _this20.QueryInterface.bulkUpdate(_this20.getTableName(options), valuesUse, options.where, options, _this20.tableAttributes).then(function (affectedRows) {
        ...
        if (options.individualHooks) {
          return Promise.map(instances, function (instance) {
            return _this20.runHooks('afterUpdate', instance, options);
          }).then(function () {
            result[1] = instances;
          });
        }
        ...
        if (options.hooks) {
          options.attributes = values;
          return _this20.runHooks('afterBulkUpdate', options).then(function () {
            delete options.attributes;
          });
        }
      });
      ...
      if (scope.include) {
        ...
        try {
          var _loop = function _loop() {
            var scopeInclude = _step10.value;

            if (scopeInclude.all || !options.include.some(function (item) {
              ...
              if (scopeInclude.as) {
                ...
                var association = scopeInclude.association || _this21.getAssociationForAlias(scopeInclude.model, scopeInclude.as);
          ...
          for (var _iterator10 = scope.include.reverse()[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
            _loop();
          }
        } catch (err) {
  ...
  _createClass(Model, [{
    ...
    value: function where(checkVersion) {
      ...
      var where = this.constructor.primaryKeyAttributes.reduce(function (result, attribute) {
        result[attribute] = _this22.get(attribute, { raw: true });
        return result;
      }, {});
    ...
    value: function get(key, options) {
      // testhint options:none
      if (options === undefined && (typeof key === 'undefined' ? 'undefined' : _typeof(key)) === 'object') {
        options = key;
        key = undefined;
      }
    ...
    value: function set(key, value, options) {
      ...
      if ((typeof key === 'undefined' ? 'undefined' : _typeof(key)) === 'object' && key !== null) {
        values = key;
        options = value || {};
    ...
    value: function save(options) {
      ...
      var primaryKeyName = this.constructor.primaryKeyAttribute;
      var primaryKeyAttribute = primaryKeyName && this.constructor.rawAttributes[primaryKeyName];
      var createdAtAttr = this.constructor._timestampAttributes.createdAt;
      var versionAttr = this.constructor._versionAttribute;
      var hook = this.isNewRecord ? 'Create' : 'Update';
      var wasNewRecord = this.isNewRecord;
      var now = Utils.now(this.sequelize.options.dialect);
      var updatedAtAttr = this.constructor._timestampAttributes.updatedAt;
      ...
      return Promise.try(function () {
        // Validate
        if (options.validate) {
          return _this25.validate(options);
        }
      }).then(function () {
        // Run before hook
        if (options.hooks) {
          var beforeHookValues = _.pick(_this25.dataValues, options.fields);
          var ignoreChanged = _.difference(_this25.changed(), options.fields); // In case of update where it's only supposed to update the passed values and the hook values
          var hookChanged = void 0;
          var afterHookValues = void 0;

          if (updatedAtAttr && options.fields.indexOf(updatedAtAttr) !== -1) {
            ignoreChanged = _.without(ignoreChanged, updatedAtAttr);
          }

          return _this25.constructor.runHooks('before' + hook, _this25, options).then(function () {
            if (options.defaultFields && !_this25.isNewRecord) {
              afterHookValues = _.pick(_this25.dataValues, _.difference(_this25.changed(), ignoreChanged));
              ...
              try {
                for (var _iterator11 = Object.keys(afterHookValues)[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
            ...
            if (hookChanged) {
              if (options.validate) {
                ...
                options.skip = _.difference(Object.keys(_this25.constructor.rawAttributes), hookChanged);
                return _this25.validate(options).then(function () {
                  delete options.skip;
                });
              }
            }
        ...
        if (!options.fields.length) return _this25;
        if (!_this25.isNewRecord) return _this25;
        if (!_this25._options.include || !_this25._options.include.length) return _this25;

        // Nested creation for BelongsTo relations
        return Promise.map(_this25._options.include.filter(function (include) {
          return include.association instanceof BelongsTo;
        }), function (include) {
          var instance = _this25.get(include.as);
          if (!instance) return Promise.resolve();

          var includeOptions = _(Utils.cloneDeep(include)).omit(['association']).defaults({
            transaction: options.transaction,
            logging: options.logging,
            parentRecord: _this25
          }).value();

          return instance.save(includeOptions).then(function () {
            return _this25[include.association.accessors.set](instance, { save: false, logging: options.logging });
          });
        });
      }).then(function () {
        var realFields = options.fields.filter(function (field) {
          return !_this25.constructor._isVirtualAttribute(field);
        });
        if (!realFields.length) return _this25;
        if (!_this25.changed() && !_this25.isNewRecord) return _this25;

        var versionFieldName = _.get(_this25.constructor.rawAttributes[versionAttr], 'field') || versionAttr;
        var values = Utils.mapValueFieldNames(_this25.dataValues, options.fields, _this25.constructor);
        var query = null;
        var args = [];
        var where = void 0;

        if (_this25.isNewRecord) {
          query = 'insert';
          args = [_this25, _this25.constructor.getTableName(options), values, options];
        } else {
          where = _this25.where(true);
          where = Utils.mapValueFieldNames(where, Object.keys(where), _this25.constructor);
          if (versionAttr) {
            values[versionFieldName] += 1;
          }
          query = 'update';
          args = [_this25, _this25.constructor.getTableName(options), values, where, options];
        }

        return _this25.sequelize.getQueryInterface()[query].apply(_this25.sequelize.getQueryInterface(), args).then(function (results) {
          var result = _.head(results);
          var rowsUpdated = results[1];

          if (versionAttr) {
            // Check to see that a row was updated, otherwise it's an optimistic locking error.
            if (rowsUpdated < 1) {
              throw new sequelizeErrors.OptimisticLockError({
                modelName: _this25.constructor.name,
                values: values,
                where: where
              });
            } else {
          ...
          try {
            for (var _iterator12 = Object.keys(_this25.constructor.rawAttributes)[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
              var attr = _step12.value;

              if (_this25.constructor.rawAttributes[attr].field && values[_this25.constructor.rawAttributes[attr].field] !== undefined && _this25.constructor.rawAttributes[attr].field !== attr) {
                values[attr] = values[_this25.constructor.rawAttributes[attr].field];
                delete values[_this25.constructor.rawAttributes[attr].field];
              }
            }
          } catch (err) {
          ...
          if (!wasNewRecord) return _this25;
          if (!_this25._options.include || !_this25._options.include.length) return _this25;
          ...
          return Promise.map(_this25._options.include.filter(function (include) {
            ...
            var instances = _this25.get(include.as);
            ...
            return Promise.map(instances, function (instance) {
              if (include.association instanceof BelongsToMany) {
                return instance.save(includeOptions).then(function () {
                  var values = {};
                  values[include.association.foreignKey] = _this25.get(_this25.constructor.primaryKeyAttribute, { raw: true });
                ...
                instance.set(include.association.foreignKey, _this25.get(_this25.constructor.primaryKeyAttribute, { raw: true }));
          ...
          // Run after hook
          if (options.hooks) {
            return _this25.constructor.runHooks('after' + hook, result, options);
          }
          ...
          try {
            for (var _iterator13 = options.fields[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
              var field = _step13.value;

              result._previousDataValues[field] = result.dataValues[field];
              _this25.changed(field, false);
            }
          } catch (err) {
    ...
    value: function reload(options) {
      ...
      return this.constructor.findOne(options).tap(function (reload) {
        if (!reload) {
        ...
        _this26._options = reload._options;
        // re-set instance values
        _this26.set(reload.dataValues, {
          raw: true,
    ...
    */
    ...
    value: function update(values, options) {
      ...
      options = Utils.cloneDeep(options);
      var setOptions = Utils.cloneDeep(options);
      setOptions.attributes = options.fields;
      this.set(values, setOptions);

      // Now we need to figure out which fields were actually affected by the setter.
      var sideEffects = _.without.apply(this, [this.changed() || []].concat(changedBefore));
      var fields = _.union(Object.keys(values), sideEffects);
    ...
    value: function destroy(options) {
      ...
      return Promise.try(function () {
        // Run before hook
        if (options.hooks) {
          return _this27.constructor.runHooks('beforeDestroy', _this27, options);
        }
      }).then(function () {
        var where = _this27.where(true);

        if (_this27.constructor._timestampAttributes.deletedAt && options.force === false) {
          var attribute = _this27.constructor.rawAttributes[_this27.constructor._timestampAttributes.deletedAt];
          var field = attribute.field || _this27.constructor._timestampAttributes.deletedAt;
          var values = {};

          values[field] = new Date();
          where[field] = attribute.hasOwnProperty('defaultValue') ? attribute.defaultValue : null;

          _this27.setDataValue(field, values[field]);

          return _this27.sequelize.getQueryInterface().update(_this27, _this27.constructor.getTableName(options), values, where, _.defaults({ hooks: false }, options)).then(function (results) {
            var rowsUpdated = results[1];
            if (_this27.constructor._versionAttribute && rowsUpdated < 1) {
              throw new sequelizeErrors.OptimisticLockError({
                modelName: _this27.constructor.name,
                values: values,
                where: where
              });
            }
          ...
          return _this27.sequelize.getQueryInterface().delete(_this27, _this27.constructor.getTableName(options), where, _.assign({ type: QueryTypes.DELETE, limit: null }, options));
        }
      }).tap(function () {
        // Run after hook
        if (options.hooks) {
          return _this27.constructor.runHooks('afterDestroy', _this27, options);
        }
      });
    }
    ...
    value: function restore(options) {
      ...
      return Promise.try(function () {
        // Run before hook
        if (options.hooks) {
          return _this28.constructor.runHooks('beforeRestore', _this28, options);
        }
      }).then(function () {
        var deletedAtCol = _this28.constructor._timestampAttributes.deletedAt;
        var deletedAtAttribute = _this28.constructor.rawAttributes[deletedAtCol];
        var deletedAtDefaultValue = deletedAtAttribute.hasOwnProperty('defaultValue') ? deletedAtAttribute.defaultValue : null;

        _this28.setDataValue(deletedAtCol, deletedAtDefaultValue);
        return _this28.save(_.extend({}, options, { hooks: false, omitNull: false }));
      }).tap(function () {
        // Run after hook
        if (options.hooks) {
          return _this28.constructor.runHooks('afterRestore', _this28, options);
        }
      });
    }
    ...
    */
    ...
    value: function increment(fields, options) {
      ...
      var updatedAtAttr = this.constructor._timestampAttributes.updatedAt;
      var versionAttr = this.constructor._versionAttribute;
      var updatedAtAttribute = this.constructor.rawAttributes[updatedAtAttr];
      ...
      var where = _.extend({}, options.where, identifier);
      ...
      try {
        for (var _iterator14 = Object.keys(values)[Symbol.iterator](), _step14; !(_iteratorNormalCompletion14 = (_step14 = _iterator14.next()).done); _iteratorNormalCompletion14 = true) {
    ...
    value: function equals(other) {
      ...
      return Utils._.every(this.constructor.primaryKeyAttributes, function (attribute) {
        return _this29.get(attribute, { raw: true }) === other.get(attribute, { raw: true });
      });
    }
