...
var Sequelize = function () {
  ...
  function Sequelize(database, username, password, options) {
    ...
    if (arguments.length === 1 && (typeof database === 'undefined' ? 'undefined' : _typeof(database)) === 'object') {
      // new Sequelize({ ... options })
      options = database;
      config = _.pick(options, 'host', 'port', 'database', 'username', 'password');
    } else if (arguments.length === 1 && typeof database === 'string' || arguments.length === 2 && (typeof username === 'undefined' ? 'undefined' : _typeof(username)) === 'object') {
  ...
  _createClass(Sequelize, [{
    ...
    value: function _import(path) {
      ...
      if (Path.normalize(path) !== Path.resolve(path)) {
        ...
        var callerPath = Path.dirname(callerFilename);
    ...
    value: function query(sql, options) {
      ...
      var bindParameters = void 0;

      return Promise.try(function () {
        options = _.assign({}, _this2.options.query, options);
        ...
        if ((typeof sql === 'undefined' ? 'undefined' : _typeof(sql)) === 'object') {
        ...
        if (options.replacements) {
          if (Array.isArray(options.replacements)) {
            sql = Utils.format([sql].concat(options.replacements), _this2.options.dialect);
          } else {
            sql = Utils.formatNamedParameters(sql, options.replacements, _this2.options.dialect);
          }
        }

        if (options.bind) {
          var bindSql = _this2.dialect.Query.formatBindParameters(sql, options.bind, _this2.options.dialect);
          sql = bindSql[0];
          bindParameters = bindSql[1];
        }

        options = _.defaults(options, {
          logging: _this2.options.hasOwnProperty('logging') ? _this2.options.logging : console.log,
          searchPath: _this2.options.hasOwnProperty('searchPath') ? _this2.options.searchPath : 'DEFAULT'
        });
        ...
        if (_this2.test._trackRunningQueries) {
          _this2.test._runningQueries++;
        }

        //if dialect doesn't support search_path or dialect option
        //to prepend searchPath is not true delete the searchPath option
        if (!_this2.dialect.supports.searchPath || !_this2.options.dialectOptions || !_this2.options.dialectOptions.prependSearchPath || options.supportsSearchPath === false) {
        ...
        return options.transaction ? options.transaction.connection : _this2.connectionManager.getConnection(options);
      }).then(function (connection) {
        var query = new _this2.dialect.Query(connection, _this2, options);

        return retry(function () {
          return query.run(sql, bindParameters).finally(function () {
            if (!options.transaction) {
              return _this2.connectionManager.releaseConnection(connection);
            }
          });
        }, Utils._.extend(_this2.options.retry, options.retry || {}));
      }).finally(function () {
        if (_this2.test._trackRunningQueries) {
          _this2.test._runningQueries--;
        }
    ...
    value: function set(variables, options) {
      ...
      // Prepare options
      options = Utils._.extend({}, this.options.set, (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' && options || {});
      ...
      // Generate SQL Query
      var query = 'SET ' + Utils._.map(variables, function (v, k) {
    ...
    value: function sync(options) {
      ...
      return Promise.try(function () {
        if (options.hooks) {
          return _this3.runHooks('beforeBulkSync', options);
        }
      }).then(function () {
        if (options.force) {
          return _this3.drop(options);
        }
      }).then(function () {
        var models = [];
        ...
        // creation order
        _this3.modelManager.forEachModel(function (model) {
          if (model) {
            models.push(model);
          } else {
        ...
        if (options.hooks) {
          return _this3.runHooks('afterBulkSync', options);
        }
      }).return(this);
    }
    ...
    value: function truncate(options) {
      ...
      var truncateModel = function truncateModel(model) {
    ...
    value: function authenticate(options) {
      ...
      return this.query(sql, Utils._.assign({ raw: true, plain: true }, options)).return();
    }
  }, {
    key: 'databaseVersion',
    ...
    value: function fn(_fn) {
      return new Utils.Fn(_fn, Utils.sliceArgs(arguments, 1));
    }
    ...
    value: function useCLS(ns) {
      ...
      if (!ns || (typeof ns === 'undefined' ? 'undefined' : _typeof(ns)) !== 'object' || typeof ns.bind !== 'function' || typeof ns.run !== 'function') throw new Error('Must provide CLS namespace');
...
Sequelize.prototype.OptimisticLockError = Sequelize.OptimisticLockError = sequelizeErrors.OptimisticLockError;
