...
var QueryGenerator = {
  ...
  createTableQuery: function createTableQuery(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= table %> (<%= attributes%>) ENGINE=<%= engine %><%= comment %><%= charset %><%= collation %><%= initialAutoIncrement %><%= rowFormat %>';
    var primaryKeys = [];
    var foreignKeys = {};
    var attrStr = [];
    ...
    if (options.uniqueKeys) {
      Utils._.each(options.uniqueKeys, function (columns, indexName) {
        if (!columns.singleField) {
          ...
          values.attributes += ', UNIQUE ' + _this.quoteIdentifier(indexName) + ' (' + Utils._.map(columns.fields, _this.quoteIdentifier).join(', ') + ')';
        }
      });
    }
  ...
  changeColumnQuery: function changeColumnQuery(tableName, attributes) {
    ...
    for (var attributeName in attributes) {
      ...
      if (definition.match(/REFERENCES/)) {
        var fkName = this.quoteIdentifier(tableName + '_' + attributeName + '_foreign_idx');
  ...
  upsertQuery: function upsertQuery(tableName, insertValues, updateValues, where, rawAttributes, options) {
    ...
    options.onDuplicate += Object.keys(updateValues).map(function (key) {
      key = _this2.quoteIdentifier(key);
  ...
  showIndexesQuery: function showIndexesQuery(tableName, options) {
  ...
  showConstraintsQuery: function showConstraintsQuery(tableName, constraintName) {
    var sql = ['SELECT CONSTRAINT_CATALOG AS constraintCatalog,', 'CONSTRAINT_NAME AS constraintName,', 'CONSTRAINT_SCHEMA AS constraintSchema,', 'CONSTRAINT_TYPE AS constraintType,', 'TABLE_NAME AS tableName,', 'TABLE_SCHEMA AS tableSchema', 'from INFORMATION_SCHEMA.TABLE_CONSTRAINTS', 'WHERE table_name=\'' + tableName + '\''].join(' ');
  ...
  attributeToSQL: function attributeToSQL(attribute, options) {
    ...
    if (attribute.references) {
      ...
      if (options && options.context === 'addColumn' && options.foreignKey) {
        ...
        var fkName = this.quoteIdentifier(options.tableName + '_' + attrName + '_foreign_idx');
   ...
   * @private
   */
  getForeignKeysQuery: function getForeignKeysQuery(tableName, schemaName) {
   ...
   * @private
   */
  getForeignKeyQuery: function getForeignKeyQuery(table, columnName) {
    var tableName = table.tableName || table;
    if (table.schema) {
      tableName = table.schema + '.' + tableName;
    }
    return 'SELECT CONSTRAINT_NAME as constraint_name' + ' FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE' + ' WHERE (REFERENCED_TABLE_NAME = ' + wrapSingleQuote(tableName) + ' AND REFERENCED_COLUMN_NAME = ' + wrapSingleQuote(columnName) + ') OR (TABLE_NAME = ' + wrapSingleQuote(tableName) + ' AND COLUMN_NAME = ' + wrapSingleQuote(columnName) + ')';
  },
