...
var QueryGenerator = {
  ...
  setSearchPath: function setSearchPath(searchPath) {
    return 'SET search_path to ' + searchPath + ';';
  },
  createSchema: function createSchema(schema) {
    return 'CREATE SCHEMA ' + schema + ';';
  },
  dropSchema: function dropSchema(schema) {
  ...
  createTableQuery: function createTableQuery(tableName, attributes, options) {
    ...
    var databaseVersion = Utils._.get(this, 'sequelize.options.databaseVersion', 0);
    var attrStr = [];
    var comments = '';
    ...
    if (options.uniqueKeys) {
      Utils._.each(options.uniqueKeys, function (columns) {
        if (!columns.singleField) {
          ...
          values.attributes += ', UNIQUE (' + columns.fields.map(function (f) {
            return _this.quoteIdentifiers(f);
          }).join(', ') + ')';
        }
      });
    }
  ...
  dropTableQuery: function dropTableQuery(tableName, options) {
    options = options || {};
  ...
  jsonPathExtractionQuery: function jsonPathExtractionQuery(column, path) {
    ...
    var pathStr = this.escape('{' + paths.join(',') + '}');
  ...
  handleSequelizeMethod: function handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Json) {
      // Parse nested object
      if (smth.conditions) {
        var conditions = _.map(this.parseConditionObject(smth.conditions), function (condition) {
  ...
  addColumnQuery: function addColumnQuery(table, key, dataType) {
    ...
    var dbDataType = this.attributeToSQL(dataType, { context: 'addColumn' });
    ...
    var query = 'ALTER TABLE ' + quotedTable + ' ADD COLUMN ' + quotedKey + ' ' + definition + ';';
  ...
  fn: function fn(fnName, tableName, body, returns, language) {
  ...
  exceptionFn: function exceptionFn(fnName, tableName, main, then, when, returns, language) {
  ...
  upsertQuery: function upsertQuery(tableName, insertValues, updateValues, where, rawAttributes, options) {
    ...
    return this.exceptionFn('sequelize_upsert', tableName, insert + ' RETURN 1;', update + '; RETURN 2', 'unique_violation', 'integer');
  },
  deleteQuery: function deleteQuery(tableName, where, options, model) {
    ...
    if (options.limit) {
      ...
      var pks = _.map(_.values(model.primaryKeys), function (pk) {
        return _this3.quoteIdentifier(pk.field);
      }).join(',');
  ...
  attributeToSQL: function attributeToSQL(attribute) {
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      if (Array.isArray(attribute.values) && attribute.values.length > 0) {
        type = 'ENUM(' + Utils._.map(attribute.values, function (value) {
  ...
  deferConstraintsQuery: function deferConstraintsQuery(options) {
    return options.deferrable.toString(this);
  },
  setConstraintQuery: function setConstraintQuery(columns, type) {
    var _this5 = this;
    ...
    if (columns) {
      columnFragment = columns.map(function (column) {
        return _this5.quoteIdentifier(column);
      }).join(', ');
    }

    return 'SET CONSTRAINTS ' + columnFragment + ' ' + type;
  },
  setDeferredQuery: function setDeferredQuery(columns) {
    return this.setConstraintQuery(columns, 'DEFERRED');
  },
  setImmediateQuery: function setImmediateQuery(columns) {
  ...
  createTrigger: function createTrigger(tableName, triggerName, eventType, fireOnSpec, functionName, functionParams, optionsArray) {
    ...
    var paramList = this.expandFunctionParamList(functionParams);
  ...
  dropTrigger: function dropTrigger(tableName, triggerName) {
    return 'DROP TRIGGER ' + triggerName + ' ON ' + tableName + ' RESTRICT;';
  },
  renameTrigger: function renameTrigger(tableName, oldTriggerName, newTriggerName) {
    return 'ALTER TRIGGER ' + oldTriggerName + ' ON ' + tableName + ' RENAME TO ' + newTriggerName + ';';
  },
  createFunction: function createFunction(functionName, params, returnType, language, body, options) {
    if (!functionName || !returnType || !language || !body) throw new Error('createFunction missing some parameters. Did you pass functionName, returnType, language and body?');

    var paramList = this.expandFunctionParamList(params);
    var indentedBody = body.replace('\n', '\n\t');
    var expandedOptions = this.expandOptions(options);

    return 'CREATE FUNCTION ' + functionName + '(' + paramList + ')\n' + ('RETURNS ' + returnType + ' AS $func$\n') + 'BEGIN\n' + ('\t' + indentedBody + '\n') + 'END;\n' + ('$func$ language \'' + language + '\'' + expandedOptions + ';');
  },
  dropFunction: function dropFunction(functionName, params) {
    if (!functionName) throw new Error('requires functionName');
    // RESTRICT is (currently, as of 9.2) default but we'll be explicit
    var paramList = this.expandFunctionParamList(params);
    return 'DROP FUNCTION ' + functionName + '(' + paramList + ') RESTRICT;';
  },
  renameFunction: function renameFunction(oldFunctionName, params, newFunctionName) {
    var paramList = this.expandFunctionParamList(params);
    return 'ALTER FUNCTION ' + oldFunctionName + '(' + paramList + ') RENAME TO ' + newFunctionName + ';';
  },
  databaseConnectionUri: function databaseConnectionUri(config) {
    var uri = config.protocol + '://' + config.user + ':' + config.password + '@' + config.host;
    if (config.port) {
      uri += ':' + config.port;
    }
    uri += '/' + config.database;
  ...
  pgEscapeAndQuote: function pgEscapeAndQuote(val) {
  ...
  expandFunctionParamList: function expandFunctionParamList(params) {
    ...
    var paramList = [];
    Utils._.each(params, function (curParam) {
      var paramDef = [];
      if (Utils._.has(curParam, 'type')) {
        if (Utils._.has(curParam, 'direction')) {
          paramDef.push(curParam.direction);
        }
        if (Utils._.has(curParam, 'name')) {
  ...
  expandTriggerEventSpec: function expandTriggerEventSpec(fireOnSpec) {
    ...
    return Utils._.map(fireOnSpec, function (fireValue, fireKey) {
      var EVENT_MAP = {
        'insert': 'INSERT',
        'update': 'UPDATE',
        'delete': 'DELETE',
  ...
  pgEnumName: function pgEnumName(tableName, attr, options) {
    options = options || {};
    var tableDetails = this.extractTableDetails(tableName, options);
    var enumName = '"enum_' + tableDetails.tableName + '_' + attr + '"';
  ...
  pgListEnums: function pgListEnums(tableName, attrName, options) {
    var enumName = '';
    var tableDetails = this.extractTableDetails(tableName, options);
  ...
  pgEnum: function pgEnum(tableName, attr, dataType, options) {
  ...
  pgEnumAdd: function pgEnumAdd(tableName, attr, value, options) {
  ...
  pgEnumDrop: function pgEnumDrop(tableName, attr, enumName) {
    enumName = enumName || this.pgEnumName(tableName, attr);
    return 'DROP TYPE IF EXISTS ' + enumName + '; ';
  },
  fromArray: function fromArray(text) {
  ...
  padInt: function padInt(i) {
    return i < 10 ? '0' + i.toString() : i.toString();
  },
  dataTypeMapping: function dataTypeMapping(tableName, attr, dataType) {
