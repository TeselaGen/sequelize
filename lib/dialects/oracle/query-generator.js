...
const QueryGenerator = {
  ...
  showSchemasQuery() {
    return 'SELECT USERNAME AS "schema_name" FROM USER_USERS WHERE USERNAME != user';
  },
  ...
  createTableQuery(tableName, attributes, options, model){
    var autoIncrement = [];

    var sqlObj = knex.schema.createTable(tableName, (table) => {
      _.each(attributes, (attr, attrName) => {
        this.createTableAttribute(table, attrName, attr);
  ...
  createTableAttribute(table, attrName, attribute) {
    var tableAttr;
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      table.specificType(attrName, template);
      return;
    } else {
      if (attribute.autoIncrement) {
        //tableAttr = table.increments(attrName);
      } else {
        //believe their setting lenght
        if (attribute.type && attribute.type.key === DataTypes.DOUBLE.key) {
          tableAttr = table.specificType(attrName, attribute.type.toSql());
        } else {
          if (attribute.type) {
            tableAttr = table.specificType(attrName, attribute.type.toString());
          } else {
            template = '';
            return;
          }
    ...
    if (attribute.type && attribute.type !== 'TEXT' && attribute.type._binary !== true &&
      ...
      tableAttr = tableAttr.defaultTo(attribute.defaultValue);
      //template += ' DEFAULT ' + this.escape(attribute.defaultValue);
    }

    if (attribute.allowNull === false) {
      tableAttr = tableAttr.notNullable();
      //template += ' NOT NULL';
    } else if (!attribute.primaryKey && !Utils.defaultValueSchemable(attribute.defaultValue)) {
      tableAttr = tableAttr.nullable();
      //template += ' NULL';
    }


    if (attribute.unique === true && !attribute.primaryKey) {
      tableAttr = tableAttr.unique();
      //template += ' UNIQUE';
    }

    if (attribute.primaryKey) {
      tableAttr = tableAttr.primary();
    ...
    if (attribute.references) {
      ...
      if (attribute.references.key) {
        tableAttr = tableAttr.references(attribute.references.key).inTable(attribute.references.model);
      } else {
        tableAttr = tableAttr.references('id').inTable(attribute.references.model);
      }

      if (attribute.onDelete) {
        tableAttr = tableAttr.onDelete(attribute.onDelete.toUpperCase()); 
  ...
  createTableQueryOld(tableName, attributes, options) {
  ...
  showTablesQuery() {
    ...
    let subQuery = 'SELECT USERNAME AS "schema_name" FROM USER_USERS';

    // if(this.dialect.options.version > 12){
    //   subQuery = 'SELECT USERNAME AS "schema_name" FROM ALL_USERS WHERE COMMON = (\'NO\')';
    // }
    ...
    const sql = ['SELECT DISTINCT table_name, lvl, owner as table_schema ',
      ...
      'FROM all_constraints a LEFT OUTER JOIN all_constraints b ON a.r_constraint_name = b.constraint_name AND a.owner = b.owner ',
      ...
      'SELECT table_name, 0 AS lvl, OWNER as TABLE_SCHEMA FROM all_tables ',
    ...
    let retSql = Utils._.template(sql)(values);
    return retSql;
  },
  ...
  dropTableQuery(tableName, isFull = true) {
    ...
    // //query.push(' DROP TABLE <%= table %> CASCADE CONSTRAINTS; ');
    ...
    var name = tableName;
    var qb = knex.schema;
    if(name.tableName) name = name.tableName;
    if(tableName.schema){
      qb = qb.withSchema(tableName.schema);
    }
    var sqlObj = qb
            .dropTable(name)
            .toSQL();
    var sql = 'begin begin execute immediate \'' + sqlObj[0].sql + " cascade constraints purge\';  exception when others then if sqlcode != -942 then raise; end if; end; " + sqlObj[1].sql + " end; "; 
    return sql;
  },
