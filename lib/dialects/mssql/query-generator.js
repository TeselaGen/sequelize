...
var QueryGenerator = _obj = {
  ...
  updateQuery: function updateQuery(tableName, attrValueHash, where, options, attributes) {
    var sql = _get(_obj.__proto__ || Object.getPrototypeOf(_obj), 'updateQuery', this).call(this, tableName, attrValueHash, where, options, attributes);
    if (options.limit) {
      var updateArgs = 'UPDATE TOP(' + this.escape(options.limit) + ')';
      sql = sql.replace('UPDATE', updateArgs);
    }
    return sql;
  },
  upsertQuery: function upsertQuery(tableName, insertValues, updateValues, where, rawAttributes) {
    var _this3 = this;

    var targetTableAlias = this.quoteTable(tableName + '_target');
    var sourceTableAlias = this.quoteTable(tableName + '_source');
  ...
  deleteQuery: function deleteQuery(tableName, where, options) {
    ...
    var limit = '';
    var query = 'DELETE<%= limit %> FROM <%= table %><%= where %>; ' + 'SELECT @@ROWCOUNT AS AFFECTEDROWS;';
  ...
  getForeignKeysQuery: function getForeignKeysQuery(table) {
    ...
    var sql = ['SELECT', 'constraint_name = C.CONSTRAINT_NAME', 'FROM', 'INFORMATION_SCHEMA.TABLE_CONSTRAINTS C', "WHERE C.CONSTRAINT_TYPE = 'FOREIGN KEY'", 'AND C.TABLE_NAME =', wrapSingleQuote(tableName)].join(' ');
  ...
  getForeignKeyQuery: function getForeignKeyQuery(table, attributeName) {
    var tableName = table.tableName || table;
    var sql = ['SELECT', 'constraint_name = TC.CONSTRAINT_NAME', 'FROM', 'INFORMATION_SCHEMA.TABLE_CONSTRAINTS TC', 'JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE CCU', 'ON TC.CONSTRAINT_NAME = CCU.CONSTRAINT_NAME', "WHERE TC.CONSTRAINT_TYPE = 'FOREIGN KEY'", 'AND TC.TABLE_NAME =', wrapSingleQuote(tableName), 'AND CCU.COLUMN_NAME =', wrapSingleQuote(attributeName)].join(' ');
  ...
  getPrimaryKeyConstraintQuery: function getPrimaryKeyConstraintQuery(table, attributeName) {
  ...
  getDefaultConstraintQuery: function getDefaultConstraintQuery(tableName, attributeName) {
    var sql = 'SELECT name FROM SYS.DEFAULT_CONSTRAINTS ' + "WHERE PARENT_OBJECT_ID = OBJECT_ID('<%= table %>', 'U') " + "AND PARENT_COLUMN_ID = (SELECT column_id FROM sys.columns WHERE NAME = ('<%= column %>') " + "AND object_id = OBJECT_ID('<%= table %>', 'U'));";
    return Utils._.template(sql)({
      table: this.quoteTable(tableName),
  ...
  selectFromTableFragment: function selectFromTableFragment(options, model, attributes, tables, mainTableAs, where) {
    var topFragment = '';
    ...
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '11.0.0')) {
      ...
      if (options.offset) {
         ...
         */
        var fragment = 'SELECT TOP 100 PERCENT ' + attributes.join(', ') + ' FROM ' + '(SELECT ' + topFragment + '*' + ' FROM (SELECT ROW_NUMBER() OVER (ORDER BY ' + orders.mainQueryOrder.join(', ') + ') as row_num, * ' + ' FROM ' + tables + ' AS ' + tmpTable + whereFragment + ')' + ' AS ' + tmpTable + ' WHERE row_num > ' + offset + ')' + ' AS ' + tmpTable;
        return fragment;
      } else {
