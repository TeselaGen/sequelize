...
var QueryGenerator = {
  ...
  extractTableDetails: function extractTableDetails(tableName, options) {
    options = options || {};
    tableName = tableName || {};
    return {
  ...
  addSchema: function addSchema(param) {
    var self = this;
  ...
  dropSchema: function dropSchema(tableName, options) {
  ...
  */
  insertQuery: function insertQuery(table, valueHash, modelAttributes, options) {
    ...
    var values = [];
    var query = void 0;
    var valueQuery = '<%= tmpTable %>INSERT<%= ignoreDuplicates %> INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)';
    var emptyQuery = '<%= tmpTable %>INSERT<%= ignoreDuplicates %> INTO <%= table %><%= output %>';
    var outputFragment = void 0;
  ...
  /*
   ...
   @private
  */
  bulkInsertQuery: function bulkInsertQuery(tableName, attrValueHashes, options, rawAttributes) {
    var _this = this;
    ...
    try {
      for (var _iterator = attrValueHashes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var attrValueHash = _step.value;
    ...
    try {
      var _loop = function _loop() {
        var attrValueHash = _step2.value;
        ...
        tuples.push('(' + allAttributes.map(function (key) {
          ...
          return _this.escape(attrValueHash[key], rawAttributes[key], { context: 'INSERT' });
        }).join(',') + ')');
      };

      for (var _iterator2 = attrValueHashes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        _loop();
      }
    } catch (err) {
    ...
    if (this._dialect.supports.updateOnDuplicate && options.updateOnDuplicate) {
      onDuplicateKeyUpdate += ' ON DUPLICATE KEY UPDATE ' + options.updateOnDuplicate.map(function (attr) {
        ...
        var key = _this.quoteIdentifier(field);
  ...
  */
  updateQuery: function updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    var values = [];
    var modelAttributeMap = {};
    var query = '<%= tmpTable %>UPDATE <%= table %> SET <%= values %><%= output %> <%= where %>';
    var outputFragment = void 0;
    var tmpTable = ''; // tmpTable declaration for trigger
  ...
  /*
   ...
   @private
  */
  arithmeticQuery: function arithmeticQuery(operator, tableName, attrValueHash, where, options) {
    attrValueHash = Utils.removeNullValuesFromHash(attrValueHash, this.options.omitNull);

    var values = [];
    var query = 'UPDATE <%= table %> SET <%= values %><%= output %> <%= where %>';
    var outputFragment = void 0;
    ...
    for (var _key in options) {
      var _value2 = options[_key];
      values.push(this.quoteIdentifier(_key) + '=' + this.escape(_value2));
    }
  ...
  /*
   ...
   @private
  */
  addIndexQuery: function addIndexQuery(tableName, attributes, options, rawTablename) {
    var _this2 = this;
    ...
    var fieldsSql = options.fields.map(function (field) {
      if (typeof field === 'string') {
        ...
        if (_this2._dialect.supports.index.collate && field.collate) {
          result += ' COLLATE ' + _this2.quoteIdentifier(field.collate);
        }
  ...
  getConstraintSnippet: function getConstraintSnippet(tableName, options) {
    var _this3 = this;

    var constraintSnippet = void 0,
        constraintName = void 0;
    ...
    switch (options.type.toUpperCase()) {
      ...
      case 'FOREIGN KEY':
        ...
        var referencesSnippet = this.quoteTable(references.table) + ' (' + this.quoteIdentifier(references.field) + ')';
  ...
  removeConstraintQuery: function removeConstraintQuery(tableName, constraintName) {
  ...
  /*
   ...
   @private
  */
  quote: function quote(collection, parent, connector) {
    var _this4 = this;
    ...
    if (typeof collection === 'string') {
      ...
      collection.forEach(function (item, index) {
        ...
        var previousAssociation = void 0;
        var previousModel = void 0;
        ...
        if (typeof item === 'string') {
          // get order index
          var orderIndex = validOrderOptions.indexOf(item.toUpperCase());

          // see if this is an order
          if (index > 0 && orderIndex !== -1) {
            item = _this4.sequelize.literal(' ' + validOrderOptions[orderIndex]);
            ...
            if (previousModel.associations !== undefined && previousModel.associations[item]) {
              ...
              if (previousModel.rawAttributes[itemSplit[0]].type instanceof DataTypes.JSON) {
                ...
                var identifier = _this4.quoteIdentifiers(previousModel.name + '.' + previousModel.rawAttributes[itemSplit[0]].field);

                // get path
                var path = itemSplit.slice(1);

                // extract path
                item = _this4.jsonPathExtractionQuery(identifier, path);
  ...
  /*
   ...
   @private
  */
  quoteIdentifiers: function quoteIdentifiers(identifiers) {
  ...
  /*
   ...
   @private
  */
  escape: function escape(value, field, options) {
    options = options || {};
  ...
  /*
   ...
   @private
  */
  selectQuery: function selectQuery(tableName, options, model) {
    var _this5 = this;
    ...
    mainTable.quotedName = !Array.isArray(mainTable.name) ? this.quoteTable(mainTable.name) : tableName.map(function (t) {
      return Array.isArray(t) ? _this5.quoteTable(t[0], t[1]) : _this5.quoteTable(t, true);
    }).join(', ');
    ...
    if (subQuery && attributes.main) {
      ...
      try {
        var _loop2 = function _loop2() {
          ...
          if (!_.find(attributes.main, function (attr) {
        ...
        for (var _iterator4 = mainTable.model.primaryKeyAttributes[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          _loop2();
        }
      } catch (err) {
    ...
    if (options.include) {
      ...
      try {
        for (var _iterator5 = options.include[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
          ...
          var joinQueries = this.generateInclude(include, { externalAs: mainTable.as, internalAs: mainTable.as }, topLevelInfo);
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        if (options.groupedLimit.on instanceof BelongsToMany) {
          ...
          if (Array.isArray(options.order)) {
            ...
            options.order.forEach(function (order, i) {
              ...
              alias = _this5.sequelize.literal(_this5.quote(alias));
        ...
        mainQueryItems.push(this.selectFromTableFragment(options, mainTable.model, attributes.main, '(' + options.groupedLimit.values.map(function (value) {
          var groupWhere = void 0;
          if (whereKey) {
          ...
          return Utils.spliceStr(baseQuery, splicePos, placeHolder.length, _this5.getWhereConditions(groupWhere, groupedTableName));
        ...
        if (subQuery) {
          ...
          _.each(mainQueryItems, function (value, key) {
            if (value.match(/^SELECT/)) {
              mainQueryItems[key] = _this5.selectFromTableFragment(options, model, attributes.main, mainTable.quotedName, mainTable.as, options.where);
            }
          });
        }
      }
  ...
  escapeAttributes: function escapeAttributes(attributes, options, mainTableAs) {
    var _this6 = this;
    ...
    return attributes && attributes.map(function (attr) {
      ...
      if (attr instanceof Utils.SequelizeMethod) {
        return _this6.handleSequelizeMethod(attr);
      }
      if (Array.isArray(attr)) {
        ...
        if (attr[0] instanceof Utils.SequelizeMethod) {
          attr[0] = _this6.handleSequelizeMethod(attr[0]);
          addTable = false;
        } else if (attr[0].indexOf('(') === -1 && attr[0].indexOf(')') === -1) {
          attr[0] = _this6.quoteIdentifier(attr[0]);
        }

        attr = [attr[0], _this6.quoteIdentifier(attr[1])].join(' AS ');
      } else {
        attr = attr.indexOf(Utils.TICK_CHAR) < 0 && attr.indexOf('"') < 0 ? _this6.quoteIdentifiers(attr) : attr;
      }
  ...
  generateInclude: function generateInclude(include, parentTableName, topLevelInfo) {
    var _this7 = this;
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      var includeAttributes = include.attributes.map(function (attr) {
        ...
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          attr = attr.map(function (attr) {
            return attr instanceof Utils.SequelizeMethod ? _this7.handleSequelizeMethod(attr) : attr;
          });
        ...
        if (verbatim === true) {
          ...
          prefix = _this7.quoteIdentifier(includeAs.internalAs) + '.' + _this7.quoteIdentifier(attr);
        }
        return prefix + ' AS ' + _this7.quoteIdentifier(includeAs.externalAs + '.' + attrAs, true);
      });
      if (include.subQuery && topLevelInfo.subQuery) {
        ...
        try {
          for (var _iterator6 = includeAttributes[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
    ...
    if (include.through) {
      ...
      if (topLevelInfo.subQuery && include.subQueryFilter) {
        ...
        var $query = this.selectQuery(include.model.getTableName(), {
          attributes: [association.identifierField],
          where: {
            $and: [associationWhere, include.where || {}]
          },
          limit: 1
        }, include.model);

        var subQueryWhere = this.sequelize.asIs(['(', $query.replace(/\;$/, ''), ')', 'IS NOT NULL'].join(' '));
    ...
    if (include.include) {
      ...
      try {
        for (var _iterator8 = include.include[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
  ...
  generateJoin: function generateJoin(include, topLevelInfo) {
    var association = include.association;
    ...
    var joinOn = this.quoteTable(asLeft) + '.' + this.quoteIdentifier(fieldLeft);
  ...
  generateThroughJoin: function generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    var _this8 = this;

    var through = include.through;
    var throughTable = through.model.getTableName();
    var throughAs = includeAs.internalAs + '->' + through.as;
    var externalThroughAs = includeAs.externalAs + '.' + through.as;
    var throughAttributes = through.attributes.map(function (attr) {
      return _this8.quoteIdentifier(throughAs) + '.' + _this8.quoteIdentifier(Array.isArray(attr) ? attr[0] : attr) + ' AS ' + _this8.quoteIdentifier(externalThroughAs + '.' + (Array.isArray(attr) ? attr[1] : attr));
    });
    var association = include.association;
    var parentIsTop = !include.parent.association && include.parent.model.name === topLevelInfo.options.model.name;
    var primaryKeysSource = association.source.primaryKeyAttributes;
    var tableSource = parentTableName;
    var identSource = association.identifierField;
    var primaryKeysTarget = association.target.primaryKeyAttributes;
    var tableTarget = includeAs.internalAs;
    var identTarget = association.foreignIdentifierField;
    var attrTarget = association.target.rawAttributes[primaryKeysTarget[0]].field || primaryKeysTarget[0];

    var joinType = include.required ? 'INNER JOIN' : 'LEFT OUTER JOIN';
    var joinBody = void 0;
    var joinCondition = void 0;
    var attributes = {
      main: [],
      subQuery: []
    };
    var attrSource = primaryKeysSource[0];
    var sourceJoinOn = void 0;
    var targetJoinOn = void 0;
    var throughWhere = void 0;
    var targetWhere = void 0;
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      try {
        for (var _iterator9 = throughAttributes[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
      ...
      if (topLevelInfo.subQuery && include.required) {
        ...
        if (topInclude.through && Object(topInclude.through.model) === topInclude.through.model) {
          query = this.selectQuery(topInclude.through.model.getTableName(), {
            ...
            model: topInclude.through.model,
            where: {
              $and: [this.sequelize.asIs([this.quoteTable(topParent.model.name) + '.' + this.quoteIdentifier(topParent.model.primaryKeyField), this.quoteIdentifier(topInclude.through.model.name) + '.' + this.quoteIdentifier(topInclude.association.identifierField)].join(' = ')), topInclude.through.where]
            },
            limit: 1,
          ...
          var join = [this.quoteTable(topParent.model.name) + '.' + this.quoteIdentifier(isBelongsTo ? topInclude.association.identifierField : topParent.model.primaryKeyAttributes[0]), this.quoteIdentifier(topInclude.model.name) + '.' + this.quoteIdentifier(isBelongsTo ? topInclude.model.primaryKeyAttributes[0] : topInclude.association.identifierField)].join(' = ');
        ...
        topLevelInfo.options.where['__' + throughAs] = this.sequelize.asIs(['(', query.replace(/\;$/, ''), ')', 'IS NOT NULL'].join(' '));
      }
    }
  ...
  getQueryOrders: function getQueryOrders(options, model, subQuery) {
    var mainQueryOrder = [];
    var subQueryOrder = [];
    ...
    if (Array.isArray(options.order)) {
      ...
      try {
        for (var _iterator10 = options.order[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
    ...
    return { mainQueryOrder: mainQueryOrder, subQueryOrder: subQueryOrder };
  },
   ...
   * @private
   */
  setAutocommitQuery: function setAutocommitQuery(value, options) {
    if (options.parent) {
      return;
    }
   ...
   * @private
   */
  addLimitAndOffset: function addLimitAndOffset(options) {
    var fragment = '';
    ...
    if (smth instanceof Utils.Where) {
      ...
      result = smth.fn + '(' + smth.args.map(function (arg) {
        if (arg instanceof Utils.SequelizeMethod) {
          return _this9.handleSequelizeMethod(arg, tableName, factory, options, prepend);
        } else if (_.isPlainObject(arg)) {
          return _this9.whereItemsQuery(arg);
        } else {
          return _this9.escape(arg);
        }
      }).join(', ') + ')';
  ...
  whereQuery: function whereQuery(where, options) {
  ...
  whereItemsQuery: function whereItemsQuery(where, options, binding) {
    var _this10 = this;
    ...
    if (_.isPlainObject(where)) {
      _.forOwn(where, function (value, key) {
        items.push(_this10.whereItemQuery(key, value, options));
      });
    } else {
  ...
  whereItemQuery: function whereItemQuery(key, value, options) {
    var _this11 = this;

    options = options || {};

    var binding = void 0;
    var outerBinding = void 0;
    var comparator = '=';
    var field = options.field || options.model && options.model.rawAttributes && options.model.rawAttributes[key] || options.model && options.model.fieldRawAttributesMap && options.model.fieldRawAttributesMap[key];
    ...
    if (key === '$or' || key === '$and' || key === '$not') {
      ...
      if (Array.isArray(value)) {
        value = value.map(function (item) {
          var itemQuery = _this11.whereItemsQuery(item, options, ' AND ');
    ...
    if (value && (value.$or || value.$and)) {
      ...
      value = value.map(function (_value) {
        return _this11.whereItemQuery(key, _value, options);
      }).filter(function (item) {
        return item && item.length;
      });
    ...
    if (_.isPlainObject(value) && fieldType instanceof DataTypes.JSON && options.json !== false) {
      var items = [];
      var traverse = function traverse(prop, item, path) {
        var where = {};
        var cast = void 0;

        if (path[path.length - 1].indexOf('::') > -1) {
          var _tmp = path[path.length - 1].split('::');
          cast = _tmp[1];
          path[path.length - 1] = _tmp[0];
        }

        var baseKey = _this11.quoteIdentifier(key);

        if (options.prefix) {
          if (options.prefix instanceof Utils.Literal) {
            baseKey = _this11.handleSequelizeMethod(options.prefix) + '.' + baseKey;
          } else {
            baseKey = _this11.quoteTable(options.prefix) + '.' + baseKey;
          }
        }

        baseKey = _this11.jsonPathExtractionQuery(baseKey, path);

        var castKey = function castKey(item) {
          var key = baseKey;
          ...
          if (cast) {
            return _this11.handleSequelizeMethod(new Utils.Cast(new Utils.Literal(key), cast));
          }
        ...
        if (_.isPlainObject(item)) {
          _.forOwn(item, function (item, prop) {
            if (prop.indexOf('$') === 0) {
              where[prop] = item;
              var _key2 = castKey(item);

              items.push(_this11.whereItemQuery(new Utils.Literal(_key2), where /*, _.pick(options, 'prefix')*/));
            } else {
          ...
          where.$eq = item;
          var _key3 = castKey(item);

          items.push(_this11.whereItemQuery(new Utils.Literal(_key3), where /*, _.pick(options, 'prefix')*/));
        }
      };
      ...
      _.forOwn(value, function (item, prop) {
        if (prop.indexOf('$') === 0) {
          var where = {};
          where[prop] = item;
          items.push(_this11.whereItemQuery(key, where, _.assign({}, options, { json: false })));
          return;
        }
    ...
    if (_.isPlainObject(value) && Object.keys(value).length > 1) {
      ...
      _.forOwn(value, function (item, logic) {
        var where = {};
        where[logic] = item;
        _items.push(_this11.whereItemQuery(key, where, options));
      });

      return '(' + _items.join(' AND ') + ')';
    }
    ...
    if (Array.isArray(value) && fieldType instanceof DataTypes.ARRAY) {
      ...
      value = value.map(function (identifier) {
        return _this11.quoteIdentifier(identifier);
      }).join('.');
    } else {
        ...
        _.forOwn(value, function (item, key) {
          if (comparatorMap[key]) {
            ...
            if (_.isPlainObject(value) && value.$any) {
              ...
              escapeValue = false;
              value = _this11.whereItemQuery(null, value);
            }
          }
        });
      }
    ...
    if (key) {
      ...
      if (key instanceof Utils.SequelizeMethod) {
        ...
        key = key.map(function (identifier) {
          return _this11.quoteIdentifier(identifier);
        }).join('.');
        prefix = false;
      } else {
  ...
  /*
   ...
   @private
  */
  getWhereConditions: function getWhereConditions(smth, tableName, factory, options, prepend) {
    var result = null;
    var where = {};
  ...
  parseConditionObject: function parseConditionObject(conditions, path) {
    var _this12 = this;

    path = path || [];
    return _.reduce(conditions, function (result, value, key) {
      if (_.isObject(value)) {
        result = result.concat(_this12.parseConditionObject(value, path.concat(key))); // Recursively parse objects
      } else {
  ...
  isIdentifierQuoted: function isIdentifierQuoted(string) {
